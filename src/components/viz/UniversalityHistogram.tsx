...
const DISTRIBUTIONS: Array<{ id: EntryDistribution; label: string }> = [
  { id: 'gaussian', label: 'Gaussian' },
  { id: 'uniform', label: 'Uniform' },
  { id: 'bernoulli', label: 'Bernoulli ±1' },
]
...
export function UniversalityHistogram() {
  const [dist, setDist] = useState<EntryDistribution>('gaussian')
  ...
  useEffect(() => {
    ...
    autoplayRef.current = window.setInterval(() => {
      setSamples((prev) => {
        ...
        const newEigs = sampleWignerEigenvalues(N, dist)
    ...
    <figure className="my-10 w-full">
      <div className="mx-auto max-w-[480px] space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-1.5">
            {DISTRIBUTIONS.map((d) => (
              <button
                ...
                className={`rounded-(--radius-button) border px-2.5 py-1 text-[12px] transition-colors ${
                  d.id === dist
          ...
          <span className="font-mono text-[11px] text-mute">N = {N}</span>
        </div>
